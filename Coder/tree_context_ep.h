...
const int tree_cont_ind_size = 21;

double evolvable_tree_context(const image& img, std::string img_name,
	...
	std::ofstream logfile(def_filename("tree", img_name, context_pop_size, predictor_pop_size, generations_cnt));
	init_fitness_fun();

	// initialise context population with random values
	vv_int context_population(context_pop_size, v_int(tree_cont_ind_size));
	for (int i = 0; i < context_pop_size; i++) {
		for (int j = 0; j < tree_cont_ind_size; j += 3) {
			do {
			...
			context_population[i][j + 2] = (int)std::floor(rnd01() * 256.0);
		}
	}
	...
	std::pair<double, v_int> best_context_values({ 0.0, v_int(tree_cont_ind_size) });
	...
	for (int g = 0; g < generations_cnt; g++) {
		...
		for (int i = 0; i < context_pop_size; i++) {
			...
			if (rnd01() < pcross) {
				...
				int p = 3 * ((int)std::floor(rnd01() * 6.0) + 1);

				v_int child1(tree_cont_ind_size);
				...
				v_int child2(tree_cont_ind_size);
			...
			// mutation
			if (rnd01() < pmut) {
				int p = (int)std::floor(rnd01() * (double)tree_cont_ind_size);
				...
				if (p % 3 == 2) {
					...
					do {
						...
						in_range = (d != 0 && context_population[i][p] + d >= 0 && context_population[i][p] + d <= 255);
					} while (!in_range);
					child[p] += d;
				}
				else {
		...
		for (int i = 0; i < context_population.size(); i++) {
			...
			best_individuals[i] = select_predictors(img, predictor_population[i], predictor_pop_size, predictor_elite_size,
				get_ctx_tree, context_population[i]);

			vv_int coeffs(contexts);
			for (int k = 0; k < contexts; k++) {
				coeffs[k] = best_individuals[i][k].second;
			}
			double entr = 1.0 / get_err_entropy(img, std::bind(evolvable, _1, _2, _3, _4, coeffs, context_population[i], get_ctx_tree));
		...
		logfile << g + 1 << ' ' << get_err_entropy(img, std::bind(evolvable, _1, _2, _3, _4, best_predictor, best_context_values.second, get_ctx_tree)) << '\n';
	}
	logfile.close();
	return get_err_entropy(img, std::bind(evolvable, _1, _2, _3, _4, best_predictor, best_context_values.second, get_ctx_tree));
}
